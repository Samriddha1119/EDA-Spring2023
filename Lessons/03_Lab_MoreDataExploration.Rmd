...
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "quatro", "cinco"), 
                                  c("un", "deux", "trois", "quatre"))) #dimneas is dimension names. First is row names and second is column names
matrix3
matrix1[4, ] #
matrix1[ , 3] #
matrix1[c(12, 14)] #get the 12th and 14th item of a matrix
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
...
list1
list1[[2]] # returns the second item from the list, list1 
...
```{r DataExercise-1a}
high_temperatures_fahrenheit<-c(64,71,72,63,51,52,61,64,67,73)
low_temperatures_fahrenheit<-c(42,49,60,46,32,32,41,47,52,54)
high_temperatures_fahrenheit
low_temperatures_fahrenheit
```
...
temp_conversion(high_temperatures_fahrenheit)
temp_conversion(low_temperatures_fahrenheit)   
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
temperature_chart<-data.frame(high_temperatures_fahrenheit,temp_conversion(high_temperatures_fahrenheit),low_temperatures_fahrenheit,temp_conversion(low_temperatures_fahrenheit))
temperature_chart
colnames(temperature_chart, do.NULL = TRUE)
colnames(temperature_chart) <- c("Fahrenheit_high","Celcius_high","Fahrenheit_low","Celcius_low")
