...
```{r}
#5.Created a series of vectors, each with four components, consisting of (a) names of students, (b) test scores out of a total 100 points, and (c) whether or not they have passed the test (TRUE or FALSE) with a passing grade of 50.
...
```{r}
#10. A function is created with 'ifelse' statement, to check whether a given test score is a passing grade of 50 or above. 
...
```{r}
#whole code is commented to avoid issues in knitting. But when executed an error is thrown as:- Error in if (x > 50) { : the condition has length > 1
